...
```{r, eval=FALSE}
my.trees <- TreeSim::sim.bd.taxa(n=300, numbsim=10, lambda=1, mu=.4, complete=FALSE)
ape::mltt.plot(my.trees, log="y", legend=FALSE) # We can see that speciation starts much more recently -- shape is more like a yule process because mu is nearing zero
my.trees <- TreeSim::sim.bd.taxa(n=300, numbsim=10, lambda=.8, mu=.2, complete=FALSE)
ape::mltt.plot(my.trees, log="y", legend=FALSE)
my.trees <- TreeSim::sim.bd.taxa(n=300, numbsim=10, lambda=.6, mu=0, complete=FALSE)
ape::mltt.plot(my.trees, log="y", legend=FALSE) 
# Not much changes as the values change and the difference is constant.
my.trees <- TreeSim::sim.bd.taxa(n=300, numbsim=10, lambda=.6, mu=0, complete=FALSE)
ape::mltt.plot(my.trees, log="y", legend=FALSE) 
my.trees <- TreeSim::sim.bd.taxa(n=300, numbsim=10, lambda=.35, mu=.25, complete=FALSE)
ape::mltt.plot(my.trees, log="y", legend=FALSE) # a lot can change, even when the sum is constant. Much longer time scales to get equal numbers of taxa when lambda and mu are close.
```
...
sim.dat[sim.dat[,2]==4,2] <- 2 #Hidden states here are 3 and 4. 4 has higher speciation rate
