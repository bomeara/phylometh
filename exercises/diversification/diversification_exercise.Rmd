...
author: "Brian O'Meara + Maggie Mamantov"
date: "3/21/2017"
...
```{r, eval=TRUE}
plot(my.tree, show.tip.label=FALSE)
```
...
**Try plotting some with other diversification parameters.**

*What happens if speciation rate is much higher than extinction rate?*
When sepeciation rate is much higher than extinction rate, the curve begins to look like a Yule model with no extinction. Also, the curves reaches the number of extant taxa more gradually when speciation rate is higher than extinction rate (grey curves) than when the two rates are close together (red curves). 

```{r, eval=TRUE}
high.trees <- TreeSim::sim.bd.taxa(n=300, numbsim=10, lambda=1, mu=0.1, complete=FALSE)
low.trees <- TreeSim::sim.bd.taxa(n=300, numbsim=10, lambda=1, mu=0.95, complete=FALSE)

depth.range <- range(unlist(lapply(high.trees,ape::branching.times)), unlist(lapply(low.trees,ape::branching.times)))
max.depth <- sum(abs(depth.range)) #ape rescales depths
plot(x=c(0, -1*max.depth), y=c(1, ape::Ntip(high.trees[[1]])), log="y", type="n", bty="n", xlab="Time", ylab="N")
colors=c(rgb(1,0,0,0.5), rgb(0, 0, 0, 0.5))
list.of.both <- list(high.trees, low.trees)
for (i in sequence(2)) {
    tree.list <- list.of.both[[i]]
    for (j in sequence(length(tree.list))) {
        ape::ltt.lines(tree.list[[j]], col=colors[[i]])   
...
legend("topleft", legend=c("b=1,d=0.95", "b=1,d=0.1"), fill=colors)
```

*How does the simulation change with different values, but keeping their difference constant?* 

Both simulations produce curves with the same, overall general shape. The simulation with higher speciation/extinction rates reaches the extant number of lineages slower(has older lineages) than the simulation with lower speciation/extinction rates even though the difference between rates is constant in both simulations. 

```{r, eval=TRUE}
my.trees1 <- TreeSim::sim.bd.taxa(n=300, numbsim=10, lambda=0.8, mu=0.7, complete=FALSE)
my.trees2 <- TreeSim::sim.bd.taxa(n=300, numbsim=10, lambda=0.2, mu=0.1, complete=FALSE)
ape::mltt.plot(my.trees1, log="y", legend=FALSE)
ape::mltt.plot(my.trees2, log="y", legend=FALSE)

depth.range <- range(unlist(lapply(my.trees1,ape::branching.times)), unlist(lapply(my.trees2,ape::branching.times)))
max.depth <- sum(abs(depth.range)) #ape rescales depths
plot(x=c(0, -1*max.depth), y=c(1, ape::Ntip(my.trees1[[1]])), log="y", type="n", bty="n", xlab="Time", ylab="N")
colors=c(rgb(1,0,0,0.5), rgb(0, 0, 0, 0.5))
list.of.both <- list(my.trees1, my.trees2)
for (i in sequence(2)) {
    tree.list <- list.of.both[[i]]
    for (j in sequence(length(tree.list))) {
        ape::ltt.lines(tree.list[[j]], col=colors[[i]])   
...
legend("topleft", legend=c("b=0.3,d=0.2", "b=0.8,d=0.7"), fill=colors)
```

*If their sum is constant?*

The shape of the curvs vary, even with holding the sum of the rates constant. The curves where the speciation rate and extinction rate are more different reaches the number extant lineages at a slow, more gradual pace than the curve were the two rates are similar. 

```{r, eval=TRUE}
my.trees3 <- TreeSim::sim.bd.taxa(n=300, numbsim=10, lambda=.5, mu=0.4, complete=FALSE)
my.trees4 <- TreeSim::sim.bd.taxa(n=300, numbsim=10, lambda=.6, mu=0.3, complete=FALSE)

depth.range <- range(unlist(lapply(my.trees3,ape::branching.times)), unlist(lapply(my.trees4,ape::branching.times)))
max.depth <- sum(abs(depth.range)) #ape rescales depths
plot(x=c(0, -1*max.depth), y=c(1, ape::Ntip(my.trees3[[1]])), log="y", type="n", bty="n", xlab="Time", ylab="N")
colors=c(rgb(1,0,0,0.5), rgb(0, 0, 0, 0.5))
list.of.both <- list(my.trees3, my.trees4)
for (i in sequence(2)) {
    tree.list <- list.of.both[[i]]
    for (j in sequence(length(tree.list))) {
        ape::ltt.lines(tree.list[[j]], col=colors[[i]])   
...
legend("topleft", legend=c("b=0.6,d=0.3", "b=0.5,d=0.4"), fill=colors)
```
...
**This doesn't seem to work with a windows computer, ls is not a recognzied command**
```{r, eval=FALSE}
...
## Your data
I'm going to use invasion stage data generated for my final project since I don't currently have empricical data. I generate it here, using the same rate matrix. I'm creating data with four invasions stages: native, not likely to invade (1); native, likely to invade (2); invader, new invasion (3); invader, old invasion (4).

```{r,eval=TRUE}
tree <- sim.bd.taxa(n=50,numbsim=1,lambda=2,mu=0.5,complete=FALSE)[[1]]
...
while(length(unique(invasion.data))!=4) {
  invasion.data <- sim.char(trees.tree, rate.matrix, model="discrete",root=1)
}

invasion.dat.true <- data.frame(tree$tip.label, tip.state=invasion.data[,,1])
invasion.dat <- data.frame(tree$tip.label, tip.state=rep(NA,50))
# Now to hide the "hidden" state -> collapsing traits into native (0) or invasive (1)
for (i in 1:50){
  if (invasion.dat.true[i,2]==1){
    invasion.dat[i,2] <- 0
  }
  if (invasion.dat.true[i,2]==2){
    invasion.dat[i,2] <- 0
  }
  if (invasion.dat.true[i,2]==3){
    invasion.dat[i,2] <- 1
  }
   if (invasion.dat.true[i,2]==4){
    invasion.dat[i,2] <- 1
  }
}
```
...
```{r}
plot(tree)
knitr::kable(cbind(invasion.dat, true.char=invasion.dat.true$tip.state))
```

Create our turnover rate and extinction fraction for a full HISSE model. I use a Yull model (all birth) for extinction fraction. 
```{r}
turnover.anc = c(1,2,3,4)
eps.anc = c(0,0,0,0)
```

Let's make our rate matrix. I'm going to remove dual rates (double transitions), but allow each rate to vary. 
```{r}
inv.trans.rates = TransMatMaker(hidden.states=TRUE)
inv.trans.rates= ParDrop(inv.trans.rates, c(3,5,8,10))
inv.trans.rates
```

Now, I can run the HISSE model. 
```{r}
inv.model <- hisse(tree, invasion.dat, f=c(1,1), hidden.states=TRUE, turnover.anc=turnover.anc, 
           eps.anc=eps.anc, trans.rate=inv.trans.rates,output.type="net.div")
inv.model
```

The output gives me net diversification of each trait. Native species with a low ability to invade have net diversification of 2.97 while native species with high ability to invade have net diversification of 1.77. New invaders have net diversification of 1.63 while old invaders have diversification of 1.08. These results indicate that being a native species increases diversification. 

Plot it.
```{r, eval=TRUE}

inv.recon <- MarginRecon(tree, invasion.dat, f=c(1,1), hidden.states=TRUE, pars=inv.model$solution, 
                       aic=inv.model$AIC)
clas(inv.recon)
plot.hisse.states(inv.recon, rate.param="net.div", show.tip.label=FALSE)
```
