...
it's a rate matrix generated under the equal rates model - the rate of transition between the two states is equal
...
And these?
it's a rate matrix generated under all rates different model - the rate of transition from 1 to 2 is twice rate of 2 to 1
...
equal rates model, because it generates a lower AIC score
...
keep1.mat <- rate.mat.gtr.4state
keep1.mat[1,2:4] <- 0
print(rayDISC(primates$tree, fourstate.data, ntraits=1, rate.mat=rate.mat.gtr.4state, node.states="marginal", model="ARD"))
print(rayDISC(primates$tree, fourstate.data, ntraits=1, rate.mat=keep1.mat, node.states="marginal", model="ARD"))

**Experiment with the effects of frequencies at the root.**
#corHMM root.p
root.yang <- corHMM(primates$tree,primates$trait[,c(1,2)],rate.cat=1,rate.mat=rate.mat.ard,node.states="marginal",root.p="yang")
print(root.yang)
#AIC 46.72994
root.maddfitz <- corHMM(primates$tree,primates$trait[,c(1,2)],rate.cat=1,rate.mat=rate.mat.ard,node.states="marginal",root.p="maddfitz")
print(root.maddfitz)
#AIC 46.2251
root.null <- corHMM(primates$tree,primates$trait[,c(1,2)],rate.cat=1,rate.mat=rate.mat.ard,node.states="marginal",root.p="null")
print(root.null)
#AIC 46.2251
