...
disc.df <-data.frame(Genus_species = row.names(disc.species), Reg = as.numeric(regime.v), x = as.numeric(disc.traits[,1]))
View(disc.df)
...
reconstruction.info <- ape::ace(disc.df$x, ult.phy.disc.tinybrlen, type="d", method="ML", CI=FALSE, model = "ER", use.eigen = TRUE)
...
recontstruction.info.cont <- ape::ace(cont.df$x, ult.phy.cont.tinybrlen, type = "continuous", method = "REML", CI = FALSE, model = "BM")

# labeled.tree <- VisualizeData(ult.phy.cont.tinybrlen, recontstruction.info.cont$ace, "ACE::CONT")

newvals <- as.vector(c(cont.traits[,1], 0, recontstruction.info.cont$ace))
VisualizeData(ult.phy.cont, newvals, "Continuous_IOB")
plot(density(newvals))
