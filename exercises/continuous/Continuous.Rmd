...
VisualizeData(ult.phy.cont, as.vector(cleaned.continuous[,1]), "Continuous_IOB")
VisualizeData(ult.phy.disc, as.vector(cleaned.discrete[,1]), "FORAGE_POOLED")
...
print(paste("The rate of evolution is", "0.006838", "in units of", "log(interorbital breadth)/time"))
```
...
regime.d.v <- as.vector(c(1,1,1,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,3,3,3,4,4,4,4,4,4))

disc.df <-data.frame(Genus_species = row.names(disc.traits.conv), Reg = as.numeric(regime.v), x = as.numeric(disc.traits[,1]))
View(disc.df)
...
reconstruction.info <- ape::ace(disc.df$x, ult.phy.disc.tinybrlen, type="discrete", method="ML", CI=FALSE, model = "ER")

# best.states <- colnames(reconstruction.info)[apply(reconstruction.info$loglik.anc, 1, which.max)]
...
recontstruction.info.cont <- ape::ace(cont.df$x, ult.phy.cont, type = "continuous", method = "REML", CI = FALSE, model = "BM")
