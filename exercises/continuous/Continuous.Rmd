...
```{r, eval=TRUE}
SumFam<-read.delim(file="SumFam2.txt", header=FALSE, stringsAsFactors = FALSE)
SumFamData <- read.delim(file="SumFam2.txt",stringsAsFactors=FALSE)
tree<-read.delim("SumFam2.txt")
phylo <- ape::rcoal(n=nrow(SumFam), tip.label = SumFam[,1])
plot(phylo)
...
```{r, eval=TRUE}
CleanData <- treedata(phylo, SumFamData) {
  result <- treedata(phylo,SumFamData)
...
```{r, eval=TRUE}
rownames(SumFam) <- SumFam[,1]
SumFam <- SumFam[,-1]
for (i in sequence(ncol(SumFam))) {
  data <- SumFam[,i]
  names(data) <- rownames(SumFam)
  phytools::contMap(phylo,data, plot=TRUE)
}
...
```{r, eval=FALSE}
cleaned.continuous <- CleanData(phylo, SumData)
