...
```{r, eval=TRUE}
tree <- ape::read.nexus(file='C:/Users/maggi/Desktop/beetleoutputtree')
...
cleaned.tree <- CleanTree(tree,discrete.data)
...
VisualizeData(cleaned.tree,cleaned.discrete)
...
rayDISC(cleaned.tree, edited.cleaned.discrete[,1:2], node.states="marginal")
rayDISC(cleaned.tree, edited.cleaned.discrete[,c(1,3)], node.states="marginal")
rayDISC(cleaned.tree, edited.cleaned.discrete[,c(1,4)], node.states="marginal")
```
...
```{r, eval = TRUE}
MKVmodel.tree <- lewisMkv(cleaned.tree, edited.cleaned.discrete)$phy
par(mfrow=c(1,2))
plot(cleaned.tree,main="Original Tree")
