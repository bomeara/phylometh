...
#cor.result <- corHMM(CleanData$phy, cor.data, rate.cat=1, node.states=c("marginal"))
#only works for 2 char. states instead of original 3
cor.result1 <- corHMM(CleanData$phy, cor.data.fake, rate.cat=1, node.states=c("marginal"))
```
...
yes, the traits are variable. can't get the lewisMkv function to work though.
```{r, eval=TRUE}
