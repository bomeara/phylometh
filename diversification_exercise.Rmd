...
```{r, eval=TRUE}
my.trees <- TreeSim::sim.bd.taxa(n=177, numbsim=10, lambda=6, mu=0.005, complete=FALSE)
...
```{r}
Tabanid.tree<-read.csv("~/Downloads/newer.tre")
ttree$tip.label
...
OTdiscrete<-read.csv("~/Documents/OTdiscrete.csv")

```
```{r}
library(corHMM)
Divmod<-corHMM(pruned.tree,OTdiscrete,rate.cat=2,rate.mat=NULL,node.states="none") 
BDmod<-birthdeath(pruned.tree)
```
```{r}

Hisse_T_rate<-hisse::TransMatMaker(hidden.states=TRUE)
Hisse_T_rate<-hisse::ParDrop(Hisse_T_rate, c(3,5,8,10))

eps.anc<- c(1,1,2,2)

Hissemod1<-hisse::hisse(pruned.tree,OTdiscrete,hidden.states=TRUE,turnover.anc=c(1,0,1,0),eps.anc,eps.anc=c(1,1,0,0),trans.rate=Divmod$solution)

Hissemod2<-hisse(pruned.tree,OTdiscrete,hidden.states=TRUE,turnover.anc=turnover.anc,eps.anc=eps.anc,trans.rate=Hisse_T_rate)
```
